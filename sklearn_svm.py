...
def lr(X,Y):
    ...
    clf_linear = svm.SVC(kernel='linear',C=15)
    clf_linear.fit(X_train_std, Y_train)



    # 5. 预测
    acc = clf_linear.score(X_train_std, Y_train)
    print(acc)
    acc = clf_linear.score(X_test_std, Y_test)
    print(acc)
    print(accuracy_score(Y_test,clf_linear.predict(X_test_std)))
    print(clf_linear.predict(X_test_std))
X,Y=get_data()
lr(X,Y)
